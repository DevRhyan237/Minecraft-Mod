...
public class CannonBallEntity extends AbstractArrow implements ItemSupplier {
	...
	public void onHitEntity(EntityHitResult entityHitResult) {
		...
		CannonBallProjectileHitsLivingEntityProcedure.execute(this.level(), this.getX(), this.getY(), this.getZ());
	}

	@Override
	public void onHitBlock(BlockHitResult blockHitResult) {
		super.onHitBlock(blockHitResult);
		CannonBallProjectileHitsBlockProcedure.execute(this.level(), blockHitResult.getBlockPos().getX(), blockHitResult.getBlockPos().getY(), blockHitResult.getBlockPos().getZ());
	}
	...
	public static CannonBallEntity shoot(Level world, LivingEntity entity, RandomSource random, float power, double damage, int knockback) {
		CannonBallEntity entityarrow = new CannonBallEntity(BetterminecraftModEntities.CANNON_BALL.get(), entity, world, null);
		...
		world.playSound(null, entity.getX(), entity.getY(), entity.getZ(), BuiltInRegistries.SOUND_EVENT.getValue(ResourceLocation.parse("betterminecraft:cannon_attack")), SoundSource.PLAYERS, 1, 1f / (random.nextFloat() * 0.5f + 1) + (power / 2));
		return entityarrow;
	}

	public static CannonBallEntity shoot(LivingEntity entity, LivingEntity target) {
		CannonBallEntity entityarrow = new CannonBallEntity(BetterminecraftModEntities.CANNON_BALL.get(), entity, entity.level(), null);
		...
		entity.level().playSound(null, entity.getX(), entity.getY(), entity.getZ(), BuiltInRegistries.SOUND_EVENT.getValue(ResourceLocation.parse("betterminecraft:cannon_attack")), SoundSource.PLAYERS, 1,
